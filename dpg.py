...
class Args:
    ...
    wandb_project_name: str = "tldr_dpg"
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(args.sft_model_path, config=model_config, trust_remote_code=True)
        ...
        with torch.no_grad():
            ...
            if args.print_sample_output_freq > 0 and update > 1 and (update - 1) % args.print_sample_output_freq == 0:
                ...
                if args.output_dir and args.num_train_epochs > 0:
                    save_dir = os.path.join(args.output_dir, f"{update}_ckpt")
            ...
            logprobs = []
            all_logprobs = [] # Needed for value computation
            ref_logprobs = []
            scores = []
            sequence_lengths = []
            for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                ...
                all_logprobs.append(all_logprob)
            ...
            del (logprob, ref_logprob, score, all_logprob)
            torch.cuda.empty_cache()
            ...
            # 6. compute retunrs
            returns = rewards * (torch.exp(-logprobs) - 1)
            returns = whiten(returns, shift_mean=False)
            ...
            for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.local_mini_batch_size, args.local_micro_batch_size):
                    with accelerator.accumulate(model):
                        ...
                        mb_all_logprobs = all_logprobs[micro_batch_inds]
                        ...
                        Q = new_logprobs - mb_logprobs
                        V = torch.mean(torch.exp(mb_all_logprobs) * (new_all_logprobs - mb_all_logprobs))
                        A = Q - V

                        # POLICY LOSS
                        loss = torch.mean((A - args.eta * mb_returns)**2)
                        ...
                        with torch.no_grad():
                            ...
                            approxkl = 0.5 * (Q**2).mean()

                            approxkl_stats[ppo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = approxkl
                            pg_loss_stats[ppo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = loss
