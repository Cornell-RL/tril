...
def generate(lm_backbone, queries, tokenizer, generation_config, reference=None, collect_rollin=False):
    """generate in a way that does not affect padding tokens"""
    logits_processor = LogitsProcessorList()
    if reference:
        ref_length = first_true_indices(reference["ref"] == tokenizer.pad_token_id)
        ref_mask = reference["ref"] != tokenizer.pad_token_id
        refs = torch.masked_fill(reference["ref"], ~ref_mask, 0)
        logits_processor.append(
            RollinProcessor(refs, reference["beta"], reference["rng"], ref_length)
        )
    ...
    if reference is not None:
        return torch.cat((queries, output.sequences[:, context_length:]), dim=1), logits_processor[0].get_rollin_mask()
...
if __name__ == "__main__":
    ...
    policy.__class__ = override_generation_routines(type(policy))
    ...
    guide_generation_config = GenerationConfig(
        max_new_tokens=args.task.response_length,
        min_new_tokens=args.task.response_length,
        temperature=(args.task.temperature + 1e-7),
        top_k=0.0,
    ...
    #rollin_loss_stats = torch.zeros(stats_shape, device=device)
    model.train()
    for update in range(1, args.ppo.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                ...
                # Collect Rollin
                query_rollin = generate(
                    accelerator.unwrap_model(model).policy,
                    query,
                    tokenizer,
                    guide_generation_config,
                    collect_rollin=True
                )
                ref = query_rollin[:, context_length:]

                # Collect Rollout
                query_response, rollin_mask = generate(
                    accelerator.unwrap_model(model).policy,
                    query,
                    tokenizer,
                    generation_config,
                ...
                scores.append(score)
                rollin_masks.append(rollin_mask.clone().detach())
            ...
            rollin_masks = torch.cat(rollin_masks, 0)
            del (logprob, ref_logprob, full_value, value, score, rollin_mask)
            torch.cuda.empty_cache()
            ...
            for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                ...
                for micro_batch_start in range(0, args.local_mini_batch_size, args.local_micro_batch_size):
                    with accelerator.accumulate(policy):
                        ...
                        # ROLLIN MASKS
                        mb_rollin_masks = ~rollin_masks[micro_batch_inds]
                        ...
                        # ROLLIN MASKS
                        pg_loss = (torch.max(pg_losses, pg_losses2) * mb_rollin_masks.to(accelerator.device)).sum()/mb_rollin_masks.sum() # mask out rollins


                        # ROLLIN REG
                        #mb_rollin_masks = ~mb_rollin_masks # mask for the rollins
                        #rollin_loss = -(new_logprobs * mb_rollin_masks.to(accelerator.device)).sum()/mb_rollin_masks.sum()

                        #loss = pg_loss + args.ppo.vf_coef * vf_loss + 0.1 * rollin_loss
                        ...
                        with torch.no_grad():
                            ...
                            # ROLLIN LOSS
                            #rollin_loss_stats[ppo_epoch_idx, minibatch_idx, gradient_accumulation_idx] = rollin_loss
        ...
        with torch.no_grad():
            ...
            # Rollin Loss Stats
            #writer.add_scalar("ppo/loss/rollin_avg", accelerator.gather(rollin_loss_stats).mean().item(), update)
