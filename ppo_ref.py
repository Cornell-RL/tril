...
def generate(lm_backbone, queries, tokenizer, generation_config, reference=None):
...
if __name__ == "__main__":
    ...
    for update in range(1, args.ppo.num_updates + 1):
        ...
        with torch.no_grad():
            ...
            references = data["reference_response_token"].to(device)
            rollin_masks = []
            for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                query = queries[i : i + args.local_rollout_forward_batch_size]
                # Reference batch
                ref = references[i : i + args.local_rollout_forward_batch_size]
